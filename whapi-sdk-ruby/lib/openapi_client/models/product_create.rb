  ...
  class ProductCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProductCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProductCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @images.length > 10
        invalid_properties.push('invalid value for "images", number of items must be less than or equal to 10.')
      end
    ...
    def valid?
      ...
      return false if @images.length > 10
      return false if @images.length < 1
      availability_validator = EnumAttributeValidator.new('String', ["in stock", "out of stock"])
    ...
    def images=(images)
      ...
      if images.length > 10
        fail ArgumentError, 'invalid value for "images", number of items must be less than or equal to 10.'
      end
    ...
    def availability=(availability)
      validator = EnumAttributeValidator.new('String', ["in stock", "out of stock"])
    ...
    def hash
      [product_retailer_id, currency, images, availability, name, url, description, price, is_hidden].hash
    end

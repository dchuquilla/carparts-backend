class User < ApplicationRecord
  ...
  enum :subscription_tier, { free: "free", paid: "paid", wait: "wait", cancel: "cancel", test: "test" }, prefix: true

  scope :paid_subscribers, -> { where(subscription_tier: %w[paid wait]) }
  scope :free_subscribers, -> { where(subscription_tier: %w[free]) }
  scope :testing_subscribers, -> { where(subscription_tier: %w[test]) }
